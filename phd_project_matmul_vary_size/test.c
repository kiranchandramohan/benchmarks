...
void multiply(int* __restrict__ A, int* __restrict__ B, int* __restrict__ C, int start, int end)
{
	int i,j,k;
	for (i = start ; i < end ; i++)
	{   
...
int verify_result(struct buffer* bufs)
{
	int pass = 1 ;
	//int i, j ;
	//int* __restrict__ C = bufs[2].bo[0]->map ;
	//
...
{
	int* __restrict__ A = buffers[0].bo[0]->map ;
	int* __restrict__ B = buffers[1].bo[0]->map ;
	int* __restrict__ C = buffers[2].bo[0]->map ;
