...
sub get_modified_fn
{
	...
	my $thread = $_[2] ;
		...
		if($line =~ m/MULTIPLYFN/) {
                        ...
                        push @new_mpy_fn_lines, $nline ;
		} elsif($line =~ m/PRINT_FINISH/) {
			my $nline = "printf(\"Finished multiply$thread\\n\") ; " ;
                        push @new_mpy_fn_lines, $nline ;
                } else {
...
for ($m3_size = 288 ; $m3_size <= 288 ; $m3_size+=$step_size) {
	for ($dsp_size = 320 ; $dsp_size <= 320 ; $dsp_size+=$step_size) {
