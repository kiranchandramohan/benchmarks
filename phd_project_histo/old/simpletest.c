...
void print_array(int array[IMG_SIZE][IMG_SIZE])
{
	int i, j ;
...
{
	int(*arra)[IMG_SIZE] =  (int(*)[IMG_SIZE])(buffers[0]->bo[0]->map) ;
	int(*arrb)[IMG_SIZE] =  (int(*)[IMG_SIZE])(buffers[1]->bo[0]->map) ;
	int(*arrc)[IMG_SIZE] =  (int(*)[IMG_SIZE])(buffers[2]->bo[0]->map) ;
...
void *local_compute(void* msg)
{
	...
	if(mesg->fd == 0) {
		init_histo((int*)buffers[1]->bo[0]->map) ;
		call_barrier(0) ;
		compute_a9_thread1(msg) ;
		call_barrier(1) ;
		compute_gray_level_mapping((int*)buffers[1]->bo[0]->map, (int*)buffers[2]->bo[0]->map) ;
		call_barrier(2) ;
		compute_image_a9_thread1(msg) ;
	} else {
		call_barrier(0) ;
		compute_a9_thread2(msg) ;
		call_barrier(1) ;
		call_barrier(2) ;
		compute_image_a9_thread2(msg) ;
	}

	return NULL ;
}

int verify_result(int* buffer1, int* buffer2, int* buffer3)
{
	int i, j ;
	int pass ;
	...
	for(i=0 ; i<HISTO_SIZE ; i++) {
		if(HISTO[i] != buffer2[i]) {                         
			printf("Difference in second buffer i=%d,j=%d : %d, %d\n",i,j,HISTO[i],buffer2[i]) ;
			pass = 0 ;                                             
		}                                                              
	}

	for(i=0 ; i<HISTO_SIZE ; i++) {
		if(GRAY_LEVEL[i] != buffer3[i]) {
			printf("Difference in third buffer i=%d,j=%d : %d, %d\n",i,j,GRAY_LEVEL[i],buffer3[i]) ;
			pass = 0 ;
		}
	}
			...
			int num_threads, int size_others)
{
	...
	if(num_threads) {
		...
		remaining_size=IMG_SIZE - size_others ;
...
int main(int argc, char *argv[])
{
    ...
    if(verify_result((int*)bufs[0]->bo[0]->map, (int*)bufs[1]->bo[0]->map, (int*)bufs[2]->bo[0]->map)) {
