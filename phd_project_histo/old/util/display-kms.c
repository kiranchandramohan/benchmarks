		...
		uint32_t *bo_handle, uint32_t *pitch)
{
	...
	if(counter == 3) {
		bo_flags |= OMAP_BO_WC ;
		printf("Write combining\n") ;
	} else {
		bo_flags |= OMAP_BO_CACHED ;
		printf("Performing caching\n") ;
	}
	*/
	...
	if (bo_flags & OMAP_BO_TILED) {
		printf("Asking for tiled memory of width=%d, height=%d\n", width, height) ;
		bo = omap_bo_new_tiled(disp->dev, width, height, bo_flags);
	} else {
		printf("Asking for memory(width=%d,height=%d,bpp=%d) of size=%d\n", width, height, bpp, width * height * bpp/8) ;
		bo = omap_bo_new(disp->dev, width * height * bpp / 8, bo_flags);
		//bo = omap_bo_new(disp->dev, width * height, bo_flags);
	}
...
connector_find_mode(struct display *disp, struct connector *c)
{
	...
	for (i = 0; i < disp_kms->resources->count_connectors; i++) {
		...
		printf("connector count modes = %d \n", connector->count_modes);
		...
		printf("connector_id=%d, c->id=%d\n", connector->connector_id, c->id);

		for (j = 0; j < connector->count_modes; j++) {
			c->mode = &connector->modes[j];
			printf("c->mode->name = %s, c->mode_str = %s\n",c->mode->name,c->mode_str) ;
...
disp_kms_open(int argc, char **argv)
{
	...
	for (i = 0; i < (int)disp_kms->connectors_count; i++) {
		...
		if (c->mode == NULL)
			continue;
		printf("Connector mode=%d,hdisplay=%d,vdisplay=%d\n",i,c->mode->hdisplay,c->mode->vdisplay) ;
