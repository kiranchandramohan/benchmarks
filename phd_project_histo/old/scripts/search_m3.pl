...
sub get_modified_fn
{
	...
	my $thread = $_[2] ;
		...
		if($line =~ m/MULTIPLYFN/) {
                        my $nline = "void multiply$thread(int a[][SIZE], int b[][SIZE], int c[][SIZE])" ;
...
for ($m3_size = 32 ; $m3_size <= 1024 ; $m3_size+=$step_size) {
	for ($dsp_size = 0 ; $dsp_size <= 0 ; $dsp_size+=$step_size) {
